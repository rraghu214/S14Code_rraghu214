...
lacks — charts, tiles, tables, timelines, notices, the approval card, and the
annotated-image overlay — are kept as clearly labelled custom extensions
(``source="custom"``). Twenty-four types in all; a student can read every one
and the validator can prove coverage.
...
COMPONENTS: dict[str, ComponentSpec] = {
    ...
    "AnnotatedImage": ComponentSpec("AnnotatedImage", {
        "src": PropSpec("text"),      # image URL, same convention as Image.src
        "alt": PropSpec("text"),
        "boxes": PropSpec("binding"),  # -> array in dataModel: [{x,y,w,h,label,confidence}, ...]
        "caption": PropSpec("text"),
    }, source="custom"),
}
